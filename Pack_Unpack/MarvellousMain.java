...
JOptionPane.showMessageDialog(this,"Number of attempts finished","Marvellous Packer Unpacker",JOptionPane.ERROR_MESSAGE);
	...
	public void run()
	{
		for(;;)//like while(true)infinait loop//its calld demon thread 
		{
			if(text2.isFocusOwner())
			{
				if( Toolkit.getDefaultToolkit().getLockingKeyState(KeyEvent.VK_CAPS_LOCK ) )//caplosloc on ahe ki off check
				{
